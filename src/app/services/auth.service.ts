...
export class AuthService {
  ...
  constructor(private mongoDBService: MongoDBService) {
    ...
    this.mongoDBService.user.subscribe(user => {
      if (!user) {
  ...
  register(userData: RegisterData): Observable<User> {
    ...
    return from(this.mongoDBService.registerEmailPassword(userData.email, userData.password)).pipe(
  ...
  private createUserProfile(userData: RegisterData): Observable<User> {
    return from(this.mongoDBService.loginEmailPassword(userData.email, userData.password)).pipe(
      switchMap(user => {
  ...
  login(loginData: LoginData): Observable<User> {
    return from(this.mongoDBService.loginEmailPassword(loginData.email, loginData.password)).pipe(
      switchMap(user => {
  ...
  getCurrentUser(): Observable<User | null> {
    const currentRealmUser = this.mongoDBService.getCurrentUser();
    if (!currentRealmUser) {
      return of(null);
    }
  ...
  logout(): Observable<void> {
    ...
    this.currentUserSubject.next(null);
    return from(this.mongoDBService.logout());
  }

  isLoggedIn(): boolean {
    return this.mongoDBService.isLoggedIn() && !!this.currentUserSubject.value;
  }
  ...
  private getUsersCollection() {
    return this.mongoDBService.getCollection(this.DATABASE_NAME, this.USERS_COLLECTION);
  }
}